...
|                  | Unrecorded cumulative funding  | $$\[F_{now} \ := F_n + p \ u\]$$                                                                                                     | The funding per base unit accumulated up to the current time, including since $$\(t_{last}\)$$.                                                                                                                                |
| $$\(j\)$$        | Last-modified index            | $$\[j \leftarrow 0\]$$ at initialization.                                                                                            | The index into $$\(F\)$$ corresponding to the event that a position was opened or modified.                                                                                                                                    |
...
| $$\(di_{max}\)$$ | Maximum funding rate of change | -                                                                                                                                    | This is an allowable funding rate change per unit of time. If a funding rate update would change it more than this, only add at most a delta of $$\(di*{max} \ (now - t*{last})\)$$. Initially, $$\(di_{max} = 30%\)$$per day. |
